...
var log = require('../utils/Logger.js').log('PaperSink'),
	PS = exports;
...
// implements a handler for JSON API requests
PS.requestHandler = function requestHandler( request, respond ){
	var status = 200, body = {};
	try {
		var req = JSON.parse( request.data );
		if( !req.action ) throw "no action specified";
		switch( req.action ){
			case 'connect':
				if( !req.roomId ) throw "no roomId specified"; 
				body.clientId = connect( req.roomId );
				respond( status, 'text/json', JSON.stringify(body) );
				break;
			case 'send':
				if( !req.clientId ) throw "no clientId specified";
				if( !req.data ) throw "no data specified";
				send( req.clientId, req.data );
				respond( status, 'text/json', JSON.stringify(body) );
				break;
			case 'poll':
				if( !req.clientId ) throw "no clientId specified";
				poll( req.clientId, function( data ){
					body.data = data;
					respond( status, 'text/json', JSON.stringify(body) );
				} );
				break;
